...
void serveMandelbrot(
    ...
    //comment out part is for ascii-brot
/*
int main (void){
...
void drawMandelbrot(
    ...
    double d;
    d = pow (2,-z);
    double height = (TILE_SIZE - 1) * d;
    struct complex s;
    s.im = center.im - 0.5 * height;
    int row = 0;
    while(s.im < center.im + 0.5 * height){
        int col = 0;
        s.re = center.re - 0.5 * height;
        
        while(s.re < center.re + 0.5 * height){
            int v = escapeSteps(s);
            pixels[row][col] = pixelColor(v);
            s.re = s.re + d;
            col ++;
        }    
        s.im = s.im + d;
...
int escapeSteps(struct complex c) {
    ...
    double temp = w.re ;
    while(steps < MAX_STEPS && moduluSquare < ESCAPE_NUM_SQUARED){
        temp = w.re;
